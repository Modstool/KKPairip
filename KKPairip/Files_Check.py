...
class FileCheck:
	...
	def download_file(K,jar_urls_and_paths):
		...
		for(L,A,M)in jar_urls_and_paths:
			...
			try:
				O='2.2';P='https://raw.githubusercontent.com/Modstool/ApkPatcher/main/setup.py'
				if(Q:=F.get(P)).status_code==200 and(R:=C.re.search('version="(.*?)"',Q.text)):
					if(S:=R.group(1))!=O:print(f"\n{C.lb}[ {C.y}Update {C.lb}]{C.c} Updating ApkPatcher to {C.g}{S}...\n\n");I=['pip','install','git+https://github.com/Modstool/ApkPatcher.git']if C.os.name=='nt'else'curl -L -o ApkPatcher.sh https://github.com/Modstool/ApkPatcher/releases/download/ApkPatcher/ApkPatcher.sh && chmod +x ApkPatcher.sh && ./ApkPatcher.sh';C.subprocess.run(I,shell=isinstance(I,str),check=H)
	...
	def F_D(A):B=[('https://github.com/Modstool/KKPairip/releases/download/Editor/APKEditor.jar',A.apkeditor_path,'758f2f9153fff96c20260b177f025a3ca3cecc9777abdd43139a17e225724612'),('https://raw.githubusercontent.com/Modstool/Objectlogger/main/Objectlogger.smali',A.Objectlogger,'ff31dd1f55d95c595b77888b9606263256f1ed151a5bf5706265e74fc0b46697')];A.download_file(B);C.os.system('cls'if C.os.name=='nt'else'clear')
	def F_D_A(B,isAPKTool,Fix_dex,isSmali):
		E=isSmali;D=Fix_dex;C=isAPKTool;A=[]
		if C or D:A.append(('https://github.com/Modstool/KKPairip/releases/download/Editor/apktool.jar',B.apktool_path,'56d59c524fc764263ba8d345754d8daf55b1887818b15cd3b594f555d249e2db'))
		if E:A.append(('https://github.com/Modstool/ApkTool_Framwork/releases/download/APKTool_2.10.0/Bak_Smali.jar',B.baksmali,'418b3861efc0f3afffb10d58a244e848d34f86cbdd3261ad2a2d1d57ee663766'))
		if C or D or E:A.append(('https://github.com/Modstool/KKPairip/releases/download/Editor/axml2xml.jar',B.axml2xml_jar_path,'e3a09af1255c703fc050e17add898562e463c87bb90c085b4b4e9e56d1b5fa62'))
